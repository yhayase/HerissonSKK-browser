export class CompositeMap<K, V> implements Map<K, V> {
    ...
    private *iterateEntries(): Generator<[K, V], void, unknown> {
        let seen = new Set<K>();
        for (const map of this.maps) {
    ...
    private *iterateKeys(): Generator<K, void, unknown> {
        for (const [key] of this.iterateEntries()) {
            yield key;
        }
    }

    private *iterateValues(): Generator<V, void, unknown> {
        for (const [, value] of this.iterateEntries()) {
            yield value;
        }
    }
    ...
    entries(): MapIterator<[K, V]> {
        return this.iterateEntries() as unknown as MapIterator<[K, V]>;
    }

    keys(): MapIterator<K> {
        return this.iterateKeys() as unknown as MapIterator<K>;
    }

    values(): MapIterator<V> {
        return this.iterateValues() as unknown as MapIterator<V>;
    }
