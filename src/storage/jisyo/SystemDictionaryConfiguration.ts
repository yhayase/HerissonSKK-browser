...
export type SystemDictionaryKind = 's' | 'l' | 'postal' | 'person' | 'place' | 'local';
...
export const SYSTEM_DICTIONARY_CATALOG: readonly SystemDictionaryDefinition[] = [
    ...
    ...(['s', 'l', 'person', 'place', 'postal'] as const).flatMap((kind) => {
        const file = { s: 'SKK-JISYO.S', l: 'SKK-JISYO.L', person: 'SKK-JISYO.jinmei', place: 'SKK-JISYO.geo', postal: 'zipcode/SKK-JISYO.zipcode' }[kind];
        const name = { s: '基本辞書 S', l: '基本辞書 L', person: '人名辞書', place: '地名辞書', postal: '郵便番号辞書' }[kind];
...
export function validateSystemDictionaries(value: unknown): SystemDictionaryDefinition[] {
    ...
    return value.map((item: unknown) => {
        ...
        if (typeof d.dictId !== 'string' || !/^[a-zA-Z0-9_-]{1,100}$/.test(d.dictId)
            ...
            || typeof d.kind !== 'string' || !['s', 'l', 'postal', 'person', 'place', 'local'].includes(String(d.kind))
