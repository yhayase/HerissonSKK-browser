...
第三者コードには各々の既存ライセンスが適用されます。SKK 辞書は配布 ZIP に同梱せず初回に取得します。取得した辞書には辞書の既存ライセンスが適用されます。現在のソースツリーにも公式辞書は含めません。アイコン等のブランド素材は本体 MIT の対象外です。公式アプリの配布・紹介に必要な利用を許可し、別製品への流用は作者の個別許可とします。[アイコンの利用条件](../public/licenses/HerissonSKK-icon.txt)を参照してください。
