...
2026-09-23 に作者から審査への送信完了の報告がありました。拡張 ID は `gnkahjfhecaokileamicaebmfkocmbng` です。審査完了と一般公開は未確認です。

提出用 ZIP と SHA256SUMS.txt は下記のローカルフォルダに保存しています。送信後に ZIP の SHA-256 を再計算し、保管したチェックサムとの一致を確認しました。ストア側の受領ファイルとのハッシュ照合は行っていません。
...
2026-09-23 に作者から、案内どおりに申告して提出したことを確認しました。

- 選択: ユーザーのアクティビティ、ウェブサイトのコンテンツ。
- 未選択: 個人を特定できる情報、健康に関する情報、財務状況や支払いに関する情報、認証に関する情報、個人的コミュニケーション、位置情報、ウェブ履歴。

この記録は作者からの報告に基づきます。入力欄のキー操作とテキストをローカル処理する機能として申告しています。任意の入力文に含まれ得る意味内容を、個別の情報収集機能として分類していません。
